  ...
  sequelize.models.Proyecto.findOne({
    ...
    .then((proyecto) => {
      if (proyecto) next();
        ...
        res
          .status(404)
          .json({ message: `El proyecto ${id_proyecto} no fue encontrado.` });
    })
    .catch((err) => handleError(req, res, err));
}
...
function buildEquipo(body, id_proyecto) {
  ...
  for (let matricula of equipo) {
    modelos.push(
      sequelize.models.Equipo.build({
        id_proyecto,
        matricula,
        estado: coordinador === matricula ? "ACEPTADO" : "PENDIENTE",
        rol: coordinador === matricula ? "EDITOR" : "VISUALIZADOR",
      })
    );
  }
...
async function createProyecto(req, res) {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "El proyecto se ha creado exitosamente." });
  } catch (err) {
...
function validateProyecto(requestType) {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (requestType === "put") skip = ["id_proyecto", "nrc", "coordinador"];
...
async function updateProyecto(req, res) {
  try {
    let proyecto = await sequelize.models.Proyecto.findOne({
...
async function deleteProyecto(req, res) {
  try {
    const proyecto = await sequelize.models.Proyecto.findOne({
      where: { id_proyecto: req.params.id_proyecto },
    });
